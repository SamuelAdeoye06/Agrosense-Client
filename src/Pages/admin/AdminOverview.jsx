...
const AdminOverview = () => {
  const { users, usersLoading, admins, savedThresholds, isSuperAdmin } = useAdmin()
  const navigate = useNavigate()

  const stats = [
    { icon: '👨🏾‍🌾', label: 'Total Farmers',    value: usersLoading ? '...' : users.length,                                     color: '#52b788' },
    { icon: '✅',      label: 'Active Farmers',   value: usersLoading ? '...' : users.filter((u) => u.status === 'active').length, color: '#2d6a4f' },
    { icon: '📅',      label: 'Total Saved Dates',value: usersLoading ? '...' : users.reduce((a, u) => a + u.saved, 0),            color: '#74c69d' },
    ...(isSuperAdmin
      ? [{ icon: '🛡️', label: 'Total Admins', value: admins.length, color: '#1b4332' }]
      : [{ icon: '🌧',  label: 'Rain Threshold', value: `${savedThresholds.rain}%`, color: '#1b4332' }]
    ),
  ]
        ...
        <div className="col-12">
          <div className="as-card">
            <div className="d-flex justify-content-between align-items-center mb-3">
              ...
              <button onClick={() => navigate('/admin/farmers')} className="btn btn-sm as-text-primary fw-bold p-0 border-0">
                ...
                <tbody>
                  {users.slice(0, 4).map((u) => (
                    <tr key={u.id}>
                      <td>
                        <div className="d-flex align-items-center gap-2 gap-md-3">
                          {u.avatarUrl ? (
                            ...
                            <div className="as-avatar-placeholder farmer-avatar-placeholder">
                              {u.name[0]}
                            </div>
                          )}
                      ...
                      <td className="as-text-soft farmer-location-cell d-none d-md-table-cell">{u.location}</td>
                      <td>
                        <span className={`as-badge ${u.status === 'active' ? 'as-badge-active' : 'as-badge-inactive'}`}>
                          {u.status === 'active' ? '● Active' : '● Inactive'}
                        </span>
                      </td>
                      <td className="as-text-accent fw-bold farmer-saved-count">{u.saved}</td>
                    </tr>
                  ))}
                  {!usersLoading && users.length === 0 && (
                    <tr><td colSpan="4" className="text-center py-4 as-text-soft">No farmers registered yet</td></tr>
                  )}
                </tbody>
              </table>
            </div>
